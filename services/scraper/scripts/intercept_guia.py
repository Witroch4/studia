"""Intercepta XHRs ao navegar para um GUIA do TecConcursos.

Descobre:
- API que resolve a URL base do guia (ex.: /guias/oab-2026 -> guia concreto)
- API que lista os cadernos-guia (com badge id, questões, "já possui")
- HTML final para mapear o link "Clique aqui para acessá-lo" -> caderno do usuário

uso: python scripts/intercept_guia.py <slug-ou-url> [--relogin]
ex.: python scripts/intercept_guia.py oab-2026
     python scripts/intercept_guia.py "oab-2026/nacional-unificado-oab/-/-"
"""
...
SKIP_HOSTS = ("facebook", "google", "doubleclick", "trackcmp", "prism.app", "analytics", "hotjar")


async def main(slug: str, relogin: bool) -> None:
    settings = get_settings()
    out = settings.discovery_dump_dir
    out.mkdir(parents=True, exist_ok=True)
    safe = slug.strip("/").replace("/", "_")
    xhrs = out / f"xhrs-guia-{safe}.jsonl"
    xhrs.write_text("")
    ...
    async with async_playwright() as p:
            ...
            try:
                ...
                if "json" in ct.lower():
                    try:
                        ...
                        body_preview = json.dumps(body, ensure_ascii=False)[:60000]
                    except Exception:
        ...
        url = slug if slug.startswith("http") else f"{settings.tc_base}/guias/{slug}"
        ...
        html_path = out / f"guia-{safe}.html"
